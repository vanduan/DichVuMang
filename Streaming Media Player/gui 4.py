...
class Ui_MainWindow(object):
    ...
    def play_video(self):
        ...
        if state:
            try:
                ...
                print 'error ==>>',vlc.libvlc_errmsg()
                player_vlc.play()
                print 'error ==>>',vlc.libvlc_errmsg()
            except:
                # Pop up Info, Could'n open media file
                msg = QtGui.QMessageBox()
                msg.setIcon(QtGui.QMessageBox.Warning)
                msg.setText('''Couldn't connect to '''+url+ '!!!')
                msg.setWindowTitle('Erroe!')
                msg.setDetailedText('Stream error !!!')
    ...
    def check_url(self ,url_check):
        ...
        if len(data) > 0:
            ...
            list_protocols=['http','mms','rtmp','rtsp','mmsh']
